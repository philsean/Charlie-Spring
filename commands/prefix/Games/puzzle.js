...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    let without = new EmbedBuilder().setCustomId('without').setLabel('Começar.').setStyle(ButtonStyle.Secondary);
    let with = new EmbedBuilder().setCustomId('with').setLabel('Começar com dicas.').setStyle(ButtonStyle.Primary);

    message.reply({ content: '**( 🧩 ) › Puzzle `( Quebra-cabeça )`**\n>>> O jogo consiste em você colocar as peças em seus lugares correspondentes, ao conseguir resolver um puzzle você ganha algo aleatório decidido pelo bot. Claro, quanto mais você joga, mais as chances de você ganhar algo bom aumenta.\n\n*Clique no botão abaixo para iniciar um puzzle, e boa sorte.*', components: [new ActionRowBuilder().addComponents(without, with)] });
  }
}